...
def my_card_def():
    ...
    try:
        ...
        def returnToMainWindow():
            ...
            if varMy_cardWindow[1] == "" or varMy_cardWindow[2] == "" or varMy_cardWindow[6] == "" or clanName == "":
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Інформація не введена в одне з обов'язкових полів!")
            elif search(r"[\\/?*]", clanName):
                Tk().withdraw()
                mesbox.showerror("Увага!", "В поле \"Прізвище роду\" введено недозволений символ!")
            elif varMy_cardWindow[6].isdigit() == False:
                Tk().withdraw()
                mesbox.showwarning("Увага!", "В поле \"Рік ...\" введено не цифри!")
            elif varMy_cardWindow[1][0] == " " or varMy_cardWindow[2][0] == " " or varMy_cardWindow[6][0] == " ":
                Tk().withdraw()
                ...
                ws['D1'] = varMy_cardWindow[1][0] + varMy_cardWindow[2][0] + varMy_cardWindow[6]
...
def addRemove_clan_def():
    # Create form and init ui
    from AddRemove_clan import Ui_AddRemoveClan
    ...
    def returnToMainWindow():
        ...
        if add_clan == "" and choice_rm_clan == "":
            ...
            Tk().withdraw()
            mesbox.showerror("Увага!", "В поле \"Додати рід\" введено недозволений символ!")
        elif choice_rm_clan != "" and len(wb.sheetnames) == 1:
            Tk().withdraw()
            mesbox.showerror("Увага!", "Якщо рід в списку тільки один, \nто його не можна видаляти!")
        else:
    ...
    try:    
        ...
        def insertingData():
            ...
            if row_index == 0:
                ...
                icon1 = QtGui.QIcon()
                icon1.addPixmap(QtGui.QPixmap("C:\\Users\\Hitar\\source\\Family_tree\\Icons/Add_image.png"),\
                ...
                ui.plainTextEdit_addinfo.setPlainText(sheet[row_index][12].value)
                
                # Output the thumbnail to the image_Button if the cell is not empty
                if sheet[row_index][0].value != None:
                    MAX_WIDTH, MAX_HEIGHT = resizeImage(sheet.title, sheet[row_index][0].value)
                    icon1 = QtGui.QIcon()
                    icon1.addPixmap(QtGui.QPixmap(sheet[row_index][0].value), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                    ui.image_Button.setIcon(icon1)
                    ui.image_Button.setIconSize(QtCore.QSize(MAX_WIDTH, MAX_HEIGHT))
                else:
                    icon1 = QtGui.QIcon()
            ...
            if count == 1:
                if ws['F1'].value != None:
                    for y in range(1, ws.max_row + 1):
                        ui.comboBox_addEdit.insertItem(y, 
        ...
        def openImage():
            ...
            if file_name != "":
                ...
                MAX_WIDTH, MAX_HEIGHT = resizeImage(ws.title, ws[line_number][0].value)
                icon1 = QtGui.QIcon()
                icon1.addPixmap(QtGui.QPixmap(ws[line_number][0].value), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                ui.image_Button.setIcon(icon1)
                ui.image_Button.setIconSize(QtCore.QSize(MAX_WIDTH, MAX_HEIGHT))
        ...
        def returnToMainWindow():
            ...
            if varAdd_editWindow[1] == "" or varAdd_editWindow[2] == "" or varAdd_editWindow[6] == "": 
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Інформація не введена в одне з обов'язкових полів!")
            elif varAdd_editWindow[6].isdigit() == False:
                Tk().withdraw()
                mesbox.showwarning("Увага!", "В поле \"Рік ...\" введено не цифри!")
            elif varAdd_editWindow[1][0] == " " or varAdd_editWindow[2][0] == " " or varAdd_editWindow[6][0] == " ":
                Tk().withdraw()
                mesbox.showerror("Увага!", "Запис в одному з обов'язкових полів некоректний!")
            else:
                ...
                if row_index == 0:
                    ...
                    i = 0
                    for row in ws.iter_rows(min_row = line_number, max_row = line_number, min_col = 5, max_col=13):
                        for cell in row:
                            cell.value = varAdd_editWindow[i]
                            i += 1

                    ws[ws.max_row][3].value = varAdd_editWindow[1][0] + varAdd_editWindow[2][0] + varAdd_editWindow[6]
                
                else:               
                    i = 0
                    for row in ws.iter_rows(min_row=row_index, max_row=row_index, min_col = 5, max_col=13):
                        for cell in row:
                            cell.value = varAdd_editWindow[i]
                            i += 1

                    ws[row_index][3].value = varAdd_editWindow[1][0] + varAdd_editWindow[2][0] + varAdd_editWindow[6]
    ...
    try:    
            ...
            if person_rowInd > 0:
                ...
                if ws[person_rowInd][2].value != None:
                    ...
                    for n in range(1, ws.max_row + 1):
                        ...
                        if parent_index == marriage_iter:                        # If they are found ...
                            if ws[n][4].value == " Чоловіча":                    # and the first is the father,
                                ui.comboBox_choiceFather.insertItem(0,           # set it to the zero position of Box.
                                            f"{ws[n][5].value} {ws[n][6].value}, {ws[n][10].value}")
                                
                                for f in range(1, ws.max_row + 1):               # After, generate all the others,
                                    if f != n:                                   # except him.
                                        ui.comboBox_choiceFather.insertItem(f, 
                            ...
                            if ws[n][4].value == " Жіноча":                    
                                ...
                                for m in range(1, ws.max_row + 1):              
                                    if m != n:                                  
                                        ui.comboBox_choiceMother.insertItem(m, 
                        ...
                        if parent_index == BS_index and n != person_rowInd:
                            ...
                            if n < 10:
                                item.setText(f"0{n}, {ws[n][5].value} {ws[n][6].value}, {ws[n][10].value}")
                            else:
                                item.setText(f"{n}, {ws[n][5].value} {ws[n][6].value}, {ws[n][10].value}")
                            BS_row += 1
                ...
                if ws[person_rowInd][1].value != None:
                    ...
                    for c in range(1, ws.max_row + 1):
                        ...
                        if marriage_index == children_index:
                            ui.tableWidget_choiceChildren.setRowCount(children_row) # generate a list of children 
                            item = QtWidgets.QTableWidgetItem()                     # in the table,
                            ui.tableWidget_choiceChildren.setItem(children_row-1, 0, item)
                            if c < 10:
                                item.setText(f"0{c}, {ws[c][5].value} {ws[c][6].value}, {ws[c][10].value}")
                            else:
                                item.setText(f"{c}, {ws[c][5].value} {ws[c][6].value}, {ws[c][10].value}")
                            children_row += 1
                        
                        marriage_iter2 = ws[c][1].value
                        if marriage_index == marriage_iter2 and c != person_rowInd:
                            ui.comboBox_choicePartner.insertItem(0,                 # and show a partner.
        ...
        def genInSomeCombo():
            ...
            if count == 1 and ws['F1'].value != None:
                for y in range(1, ws.max_row + 1):
                    ui.comboBox_choicePerson.insertItem(y, 
                                f"{ws[y][5].value} {ws[y][6].value}, {ws[y][10].value}")

                    ui.comboBox_choiceBS.insertItem(y-1, 
                                f"{ws[y][5].value} {ws[y][6].value}, {ws[y][10].value}")
                    
                    ui.comboBox_choiceChildren.insertItem(y-1, 
                                f"{ws[y][5].value} {ws[y][6].value}, {ws[y][10].value}")
        ...
        def addBS():
            ...
            BS_index = ui.comboBox_choiceBS.currentIndex() + 1                      # Get the row index in the file,
            ...
            if BS_index < 10:                                                       # The person index must consist 
                item.setText(f"0{BS_index}, {ws[BS_index][5].value} {ws[BS_index][6].value}, {ws[BS_index][10].value}")                     
            else:
                item.setText(f"{BS_index}, {ws[BS_index][5].value} {ws[BS_index][6].value}, {ws[BS_index][10].value}")
        ...
        def addChildren():
            ...
            if Children_index < 10:
                item.setText(f"0{Children_index}, {ws[Children_index][5].value} {ws[Children_index][6].value}, {ws[Children_index][10].value}")
            else:
                item.setText(f"{Children_index}, {ws[Children_index][5].value} {ws[Children_index][6].value}, {ws[Children_index][10].value}")
        ...
        def returnToMainWindow():
            ...
            if rowCount_BS > 0:
                for t in range(rowCount_BS):
                    if ui.tableWidget_BS.item(t, 0).text() != "":
                        varFam_t_tab_BS.append(ui.tableWidget_BS.item(t, 0).text())
            
            if rowCount_Children > 0:
                for t in range(rowCount_Children):
                    if ui.tableWidget_choiceChildren.item(t, 0).text() != "":
                        varFam_t_tab_Chd.append(ui.tableWidget_choiceChildren.item(t, 0).text())
                ...
                for b in range(len(varFam_t_tab_BS)):
                    for c in range(len(varFam_t_tab_Chd)):
                        if varFam_t_tab_BS[b] == varFam_t_tab_Chd[c]:
                            Tk().withdraw()
                            mesbox.showerror("Увага!", "В таблиці \"Брати/Сестри\" і \"Діти\" \
                                \nзнайдено однакові елементи!")
                            return
        
            if choicePerson_ind == 0:
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Не вибрано, для кого встановити \nсімейні рв\'язки!")
            ...
            elif choiceFather_text == choiceMother_text and choiceFather_ind != 0:
                Tk().withdraw()
                mesbox.showerror("Увага!", "Вибрано однакових батьків!")
            elif choiceFather_ind != 0 and ws[choiceFather_ind][10].value > ws[choicePerson_ind][10].value:
                Tk().withdraw()
                mesbox.showerror("Увага!", "В поле \"Батько\" встановлено некоректну особу!")
            elif choiceMother_ind != 0 and ws[choiceMother_ind][10].value > ws[choicePerson_ind][10].value:
                Tk().withdraw()
                mesbox.showerror("Увага!", "В поле \"Мати\" встановлено некоректну особу!")
            elif choiceFather_ind != 0 and choiceMother_ind != 0 and \
                 ws[choiceFather_ind][1].value != ws[choiceMother_ind][1].value:
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Батько або Мати вже в шлюбі!")
            elif choicePartner_text == choicePerson_text:
                Tk().withdraw()
                mesbox.showerror("Увага!", "Поля \"Встановити сімейні зв\'зки для\" \
                                    \n і \"Партнер\" не можуть бути однакові!")
            elif varFam_t_tab_BS != [] and choiceFather_text == "" or choiceMother_text == "":
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Не можна додавати братів чи сестер, \nпоки не вибрано батьків!")
            elif varFam_t_tab_Chd != [] and choicePartner_ind == 0 and choicePartner_text == "":
                Tk().withdraw()
                mesbox.showwarning("Увага!", "Не можна додавати дітей, \nпоки не вибрано партнера!")
                ...
                def setChildren(marriage):
                    ...
                    if varFam_t_tab_Chd != []:
                        for c in range(len(varFam_t_tab_Chd)):                  # go through the list with children,
                            chd_row_ind = int(varFam_t_tab_Chd[c][0:2])         # and everyone read the index 
                                                                                # of the string in the file.
                            # If the child's year of birth is less than the person's - show error
                            if int(ws[chd_row_ind][10].value) < int(ws[choicePerson_ind][10].value):
                                Tk().withdraw()
                                mesbox.showerror("Увага!", f"{ws[chd_row_ind][5].value} {ws[chd_row_ind][6].value} \
                                                \nне може бути Вашою дитиною!")
                            elif ws[chd_row_ind][2].value != None and ws[chd_row_ind][2].value != marriage:
                                Tk().withdraw()
                                mesbox.showerror("Увага!", f"{ws[chd_row_ind][5].value} {ws[chd_row_ind][6].value} \
                ...
                if choicePartner_ind == 0 and choicePartner_text != "":
                    ...
                    if ws[choicePerson_ind][1].value != None:
                        ...
                        marriage = person_ind + partner_ind                     # add new person and partner indexes,
                        ...
                        for p in range(1, ws.max_row+1):
                            ...
                            if old_marriage == parents_ind:                     # overwrite the new index
                                ws[p][2].value = marriage                       # for existing children,
                        
                        ws[choicePerson_ind][1].value = marriage                # and set a new index in the 
                        ws[choicePartner_ind][1].value = marriage               # marriage cells.
                        setChildren(marriage)
                ...
                def setBS(marriage, father_row_ind):
                    ...
                    if varFam_t_tab_BS != []:
                        for s in range(len(varFam_t_tab_BS)):                   # go through the list with siblings,
                            BS_row_ind = int(varFam_t_tab_BS[s][0:2])           # and everyone read the index
                                                                                # of the string in the file.    
                            # If the siblings year of birth is less than the person's - show error.
                            if int(ws[BS_row_ind][10].value) < int(ws[father_row_ind][10].value):
                                Tk().withdraw()
                                mesbox.showerror("Увага!", f"{ws[BS_row_ind][5].value} {ws[BS_row_ind][6].value} \
                                \nне може бути Вашим братом чи сестрою!")
                            elif ws[BS_row_ind][2].value != None and ws[BS_row_ind][2].value != marriage:
                                Tk().withdraw()
                                mesbox.showerror("Увага!", f"{ws[BS_row_ind][5].value} {ws[BS_row_ind][6].value} \
                                \nвже є чиїмось братом чи сестрою!")
                            elif BS_row_ind == choicePerson_ind or BS_row_ind == choicePartner_ind or \
                                BS_row_ind == choiceFather_ind or BS_row_ind == choiceMother_ind or \
                                BS_row_ind == ui.comboBox_choiceFather.findText("") or \
                                BS_row_ind == ui.comboBox_choiceMother.findText("") or \
                                BS_row_ind == ui.comboBox_choicePartner.findText(""):
                                Tk().withdraw()
                                mesbox.showerror("Увага!", "Особа в списку \"Брати/Сестри\" співпадає \
                ...
                if (choiceFather_ind != 0 and choiceFather_text != "") and \
                    ...
                    if ws[choicePerson_ind][2].value != None and ws[choicePerson_ind][2].value != marriage:
                        Tk().withdraw()
                        mesbox.showerror("Увага!", f"{ws[choicePerson_ind][5].value} {ws[choicePerson_ind][6].value} \
                                        \nвже є чиїмось братом чи сестрою!")
                    else:
                ...
                # If either the father or the mother has changed
                    ...
                    if choiceFather_ind == 0 and choiceMother_ind != 0:
                        father_ind = ws[ui.comboBox_choiceFather.findText("")][3].value
                        mother_ind = ws[choiceMother_ind][3].value
                        old_marriage =  ws[ui.comboBox_choiceFather.findText("")][1].value
                    else:
                        father_ind = ws[choiceFather_ind][3].value
                        mother_ind = ws[ui.comboBox_choiceMother.findText("")][3].value
                        old_marriage =  ws[ui.comboBox_choiceMother.findText("")][1].value
                    
                    marriage = father_ind + mother_ind                     # add new person and partner indexes,
                    ...
                    if choiceFather_ind == 0 and choiceMother_ind != 0:
                        ws[ui.comboBox_choiceFather.findText("")][1].value = marriage 
                        ws[choiceMother_ind][1].value = marriage
                        father_row_ind = ui.comboBox_choiceFather.findText("") 
                    else:
                        ws[choiceFather_ind][1].value = marriage
                        ws[ui.comboBox_choiceMother.findText("")][1].value = marriage
...
def review_def():
    ...
    try:
        ...
        def genInTable():
            ...
            row_ind = col_ind = 0
            for row_cells in ws.iter_rows(min_col = 5, max_col = 13):           # Read one row from the file,
                for cell in row_cells:                                          # and write in one cell of 
                    item = QtWidgets.QTableWidgetItem()                         # the table one value from 
                    ui.tableWidget.setItem(row_ind, col_ind, item)              # the read tuple (row).
